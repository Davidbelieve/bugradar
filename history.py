...
def get_history(repo_id: int | None = Query(default=None), limit: int = Query(default=100, le=500), current_user: dict = Depends(get_current_user)):
    ...
    with _engine.connect() as conn:
        if repo_id:
            ...
            rows = conn.execute(_text("""
                SELECT id, risk_score, high_risk_functions, repo_id, scanned_at
                FROM scan_reports WHERE repo_id=:rid ORDER BY scanned_at DESC LIMIT :lim
            """), {"rid": repo_id, "lim": limit}).fetchall()
        else:
            rows = conn.execute(_text("""
                SELECT sh.id, sh.risk_score, sh.high_risk_functions, sh.repo_id, sh.timestamp
    ...
    return [{"id": r[0],"risk_score": r[2], "high_risk_functions": r[3], "repo_id": r[4], "scanned_at": str(r[5]) if r[5] else None} for r in rows]
